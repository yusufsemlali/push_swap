...
void	ft_sort_small(t_stack **a)
{
	...
	while (!is_sorted(*a))
	{
		if ((*a)->number > (*a)->next->number
			&& (*a)->next->number != min_value(*a))
			sa(a);
		else if ((*a)->number == max_value(*a))
			rra(a);
		else if ((*a)->number != min_value(*a) || ((*a)->number == min_value(*a)
				&& (*a)->next->number != second_min_value(*a)))
			ra(a);
		else
			rra(a);
	}
...
// this another algo that is going to be usefull in the future or not ,
this project if fuckign annoying.
